...
respectively. Let $0 \leq n_{11} \leq \min(n_1,n_2)$ be the number of wrong numbers which were found by both statisticians. 
...
testP
```
i.e. $n_1=`r testP[1]`$, $n_2=`r testP[2]`$ and $n_{11}=`r testP[3]`$. 
Hence, the total number of errors found so far is $S=`r sum(testP)`$.
...
```
In other words, the estimated total number of errors is `r M.hat["Petersen"]`. The 95% confidence interval (CI) for $M$ is `r sprintf("(%.0f,%.0f)",M.hat["cil"],M.hat["ciu"])` -- see the package documentation for details on the method for computing the (CI). To verify the computations one could alternatively compute the Lincoln-Petersen estimator manually:
...
The method is based on the assumption that the two statisticians are equally good at spotting errors, however, errors have heterogeneous probabilities to be detected. No specific parametric model for the detection is although required. An R implementation of the estimator is readily available as part of the [`SPECIES`](https://cran.r-project.org/web/packages/SPECIES/index.html) package.
...
One limitation of the above is that we have used a guesstimate on how  the detectionprobability $p_n$ evolves over time. An improvement would be to sequentially estimate this parameter from the obtained results.
This goes along the lines of the work by Dalal and Mallows (1988) on when to stop testing your software described in the following [note](https://htmlpreview.github.io/?https://github.com/hoehleatsu/When2Stop/blob/master/when2stop.html).
...
```
In other words, one should test for $n_{\text{stop}}=`r df %>% filter(rank(Yn) == 1) %>% select(n)`$ rounds.
